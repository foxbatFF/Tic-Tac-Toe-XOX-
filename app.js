...
function chooseArea(block) {
    if (block.textContent === "") {
        ...
        if (player === "O") {
            block.style.color = "orangered";
        }
        turnPlayer();
    }
    else {
        errorText.textContent = "Heyi, it's not empty"
        block.style.border = "4px solid red"
        setTimeout(() => {
            errorText.textContent = "";
            block.style.border = "1px solid tomato"
        }, 2500);
    }
    checkWin();
    checkTie();
    ...
    if (gameOver) {
        playerText.textContent = `Game is over, ${winner} Win :D`
